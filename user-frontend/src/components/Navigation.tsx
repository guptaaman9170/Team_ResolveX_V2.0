        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              {/* Notifications */}
              <Button variant="ghost" size="sm" className={`relative ${isHighContrast ? buttonHighContrastStyle : ""}`} onClick={() => setIsNotificationsOpen((prev) => !prev)}>
              ...
              <Button className={`btn-civic ${isHighContrast ? buttonHighContrastStyle : ""}`}>
            ...
            <div className="md:hidden">
              <Button variant="ghost" size="sm" className={`${isHighContrast ? buttonHighContrastStyle : ""}`} onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Toggle menu">
            ...
            <div className={`md:hidden py-4 border-t ${isHighContrast ? "border-blue-400" : "border-border"}`}>
              <div className="flex flex-col space-y-3">
                ...
                <div className="flex items-center justify-between px-3 py-2">
                  <Button variant="ghost" size="sm" className={isHighContrast ? buttonHighContrastStyle : ""} onClick={() => changeLanguage(language === "en" ? "hi" : "en")}>
                    <Globe className="w-4 h-4 mr-2" />
                    {navigationData[language].language}
                  </Button>

                  <Button variant="ghost" size="sm" className={isHighContrast ? buttonHighContrastStyle : ""} onClick={() => toggleAccessibility("contrast")}>
                    <Eye className="w-4 h-4" />
                  </Button>
                </div>

                <div className="px-3">
                  <Button className={`btn-civic w-full ${isHighContrast ? buttonHighContrastStyle : ""}`}>
                    {navigationData[language].signOut}
                  </Button>
                </div>
              </div>
